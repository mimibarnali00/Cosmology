...
def potential(phi,potparams):
	...
	Gw = potparams[5]
	Rw = potparams[6]
	t = np.exp(2.0/np.sqrt(3.0)*phi)
	v = (W0**2/Vv**3)*(-Cw/np.sqrt(t) + Aw/(np.sqrt(t) - Bw) + (t/Vv)*(-Gw/(1.0 + Rw*t**(1.5)/Vv))) + 3.5e-10
	dvdphi = ((W0**2)/(np.sqrt(3.0)*Vv**3))*(Cw/np.sqrt(t) - (Aw*np.sqrt(t))/(Bw - np.sqrt(t))**2 + (t*Gw*((t)**(1.5)*Rw -2.0*Vv))/((t)**(1.5)*Rw + Vv)**2)
	return v,dvdphi
...
potparams = np.zeros(7)
potparams[0] = 9.4690334348   #W0
potparams[1] = 0.04   #Cw
potparams[2] = 0.02   #Aw
potparams[3] = 1.0   #Bw
potparams[4] = 1000   #Vv
potparams[5] = 3.080548e-5*potparams[4]   #Gw
potparams[6] = 7.071067e-4*potparams[4]   #Rw
...
print("Ne = ",NiNe(kp)[5])
#print(np.amax(NiNe(kp)[6]),np.amin(NiNe(kp)[6]))
#############
