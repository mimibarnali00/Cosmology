...
Gkin = ((np.exp(-i*kp*eta)/(np.sqrt(2*kp)))/z)[NiNe(kp)[1]]
dGkin = (((-i*kp)*np.exp(-i*kp*eta)/(np.sqrt(2*kp)))/z - (np.exp(-i*kp*eta)/(np.sqrt(2*kp)))*dz/(z**2))[NiNe(kp)[1]]

#initial conditions for tensors
hkin = ((np.exp(-i*kp*eta)/(np.sqrt(2*kp)))/anew)[NiNe(kp)[1]]
dhkin = (((-i*kp)*np.exp(-i*kp*eta)/(np.sqrt(2*kp)))/anew - (np.exp(-i*kp*eta)/(np.sqrt(2*kp)))/anew)[NiNe(kp)[1]]
...
Grsol = solve_ivp(scalarperturbeq,[NiNe(kp)[2],NiNe(kp)[5]],[np.real(Gkin),np.real(dGkin)],t_eval=NiNe(kp)[6],args = (kp, )) #default RK45
#print(Grsol)
Nrfin = Grsol.t
Gr = Grsol.y[0]
dGr = Grsol.y[1]
...
#############################################
Gisol = solve_ivp(scalarperturbeq,[NiNe(kp)[2],NiNe(kp)[5]],[np.imag(Gkin),np.imag(dGkin)],t_eval=NiNe(kp)[6],args = (kp, )) #default RK45
#print(Gisol)
Nifin = Gisol.t
Gi = Gisol.y[0]
dGi = Gisol.y[1]
...
hrsol = solve_ivp(tensorperturbeq,[NiNe(kp)[2],NiNe(kp)[5]],[np.real(hkin),np.real(dhkin)],t_eval=NiNe(kp)[6],args = (kp, )) #default RK45
#print(hrsol)
Nhrfin = hrsol.t
hr = hrsol.y[0]
dhr = hrsol.y[1]
...
#############################################
hisol = solve_ivp(tensorperturbeq,[NiNe(kp)[2],NiNe(kp)[5]],[np.imag(hkin),np.imag(dhkin)],t_eval=NiNe(kp)[6],args = (kp, )) #default RK45
#print(hisol)
Nhifin = hisol.t
hi = hisol.y[0]
dhi = hisol.y[1]
